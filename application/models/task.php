<?php
  class Task extends CI_Model{
      ...
      //根据部门id、开始时间和结束时间来获取任务信息
      function getTaskByDptid_gantt($dptId,$displayAll,$startDate,$endDate){
          if($displayAll ==1){
                $sql='select * from task where (dptid ='.$dptId .') and ((startdate <="' .$startDate.'" and enddate >="'.
                                        $startDate.'") or (startdate >="'.$startDate.'" and startdate <="'.$endDate.'")) order by tasktypeid asc,startdate asc';  
          }else{
                $sql='select * from task where (dptid ='.$dptId .') and (importance =1) and ((startdate <="' .$startDate.'" and enddate >="'.
                                        $startDate.'") or (startdate >="'.$startDate.'" and startdate <="'.$endDate.'")) order by tasktypeid asc,startdate asc';
          }
      ...
      function getTaskByUserid($userId,$startDate,$endDate){
          ...
          $sql='select * from task where (userid ='.$userId .') and (enrolldate >="' .$startDate.'" and enrolldate <="'.
                                  $endDate.'" ) order by tasktypeid asc,startdate desc  ';
          $query= $this->db->query($sql);
          $rows =  $query->result_array();
          for($i=0;$i<count($rows);$i++){
              $rows[$i]['username'] = $this->User[$rows[$i]['userid']]; 
      ...
      function countOfTaskBySearch($type,$content,$startDate,$endDate){
           if($type == "content"){
               $sql='select count("taskid") from task where (taskcontent like "%'.$content .'%") and ((startdate <="' .$startDate.'" and enddate >="'.
                                        $startDate.'") or (startdate >="'.$startDate.'" and startdate <="'.$endDate.'"))';  
           }else if($type=='user'){
               $sql='select count("taskid") from task where (responsible like "%'.$content .'%") and ((startdate <="' .$startDate.'" and enddate >="'.
                                        $startDate.'") or (startdate >="'.$startDate.'" and startdate <="'.$endDate.'"))'; 
           }
           //file_put_contents('c:\error.txt',$sql);
           $query= $this->db->query($sql);
 ...
 // file_put_contents('c:\error.txt',$sql);
      ...
      function newTask($userid,$tasktypeid,$taskimportance,$startdate,$enddate,$taskcontent,$responsible,$enrolldate){
          $sql = 'select * from user where userid ='.$userid;
          $query = $this->db->query($sql);
          $rows= $query->result_array();
          $sql = 'insert into task values (NULL,"'.$taskcontent.'","'.$startdate.'","'.$enddate.'",'.$taskimportance.',0,'.$tasktypeid.','.$rows[0]['dptid'].','.$userid.',"'.$responsible.'","'.$enrolldate.'")' ;
          //file_put_contents('c:\error.txt',$sql);
          $query = $this->db->query($sql);
          return $query;
      }
